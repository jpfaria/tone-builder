...
.venv/bin/tone-builder library record <guitar> <position> <string|all> --device "<interface>" --channel <n>   # listens: names each note as it lands, ends when the string is complete; a wrong note is played again. `all` = the six strings in a row, 6 first. `--seconds N` = a fixed-length take

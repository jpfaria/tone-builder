...
- `battery.run_battery(baseline: Renderer, candidates: list[Candidate], assignments: list[dict], research: dict, workdir: Path) -> tuple[dict[str, dict], dict]` — (resultado por classe, medição da linha de base); por classe: `{"status": "measured"|"not_tested"|"no_candidate", "sourced": decide(...)|None, "unsourced_best": {"name","deviation"}|None, "reason": str|None}`. Compressor com `gain_reduction_db` None ou < 3 → excluído e, sem sobrar nenhum, `not_tested` com motivo. Candidato é "com fonte" quando `unit` ∈ `sourced_units(research)[klass]`. Classe sem candidato → `no_candidate`, motivo tirado de `research["not_found"]` quando houver, senão `None`.
